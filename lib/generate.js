...
module.exports = (templateSrc, root, blueprint, name, args, options={}) => {
  ...
  if (!fs.existsSync(src)) {
    return console.log(chalk.red(
      `Blueprint '${blueprint}' does not exist in ${templateSrc}` +
      `\nSee 'blue list'`
    ))
  }
  ...
  if (fs.existsSync(generateSrc)) {
    ...
    config = file.readJson(path.join(src, 'config.json')) || {}
  ...
  //typeof hooks.init === 'function' && hooks.init(context, metalsmith)
  ...
  //typeof hooks.before === 'function' && hooks.before(context, metalsmith)

  if (options.preview) {
    console.log(`Preview: ${blueprint} ${name}\n`)

    metalsmith.process(function(err) {
      if (err) throw err
    })
  } else {
    console.log(chalk.cyan(`${blueprint} ${name}\n`))

    metalsmith.build(function(err) {
      if (err) throw err
      console.log(chalk.cyan('\nDone!'))
      //typeof hooks.after === 'function' && hooks.after(context, metalsmith)
    })
  }
}
