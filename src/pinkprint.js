...
exports.default = (src, dest, context, argv) => {
  var { src, config, hooks, loaded } = loadPluginFiles(src);

  if (loaded && argv.verbose) {
  ...
  var metalsmith = Metalsmith(dest)
    ...
    .use(plugins.filename(argv.verbose));
  ...
  /*if (argv.skipOverwrites) {
    metalsmith.use(plugins.skipOverwrites(dest))
  }*/

  metalsmith.use(plugins.template('ejs', argv.verbose));
